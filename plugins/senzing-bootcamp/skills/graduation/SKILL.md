...
- **Content check (optional, non-blocking):** run the script with `--check` to confirm each recap module section present carries the four required subsections; add `--expect-modules "<module names from modules_completed>"` to also flag any completed module missing its section entirely. (Whole-module presence is primarily guaranteed by the Step 1a reconcile, which runs first.) If it reports gaps, backfill per 1a and re-render. A gap never blocks graduation.
