...
except Exception:  # pragma: no cover - falls back to inlined brand values
    EMBER = (245, 120, 38)
    DARK_INK = (24, 22, 15)
    BODY_INK = (74, 70, 64)
...
def render_with_fpdf2(doc: Discoveries, output: Path) -> bool:
    ...
    try:
        ...
        pdf.multi_cell(epw, 10, _safe(doc.title or "Data Discoveries"))
        pdf.set_font("Helvetica", "", 12)
        pdf.set_text_color(*EMBER)
        pdf.multi_cell(epw, 7, "What Senzing found in your data")
        pdf.ln(2)

        pdf.set_text_color(*BODY_INK)
        pdf.set_font("Helvetica", "", 10.5)
        for key, value in doc.meta:
            pdf.multi_cell(epw, 5.5, _safe(f"{key}: {_md_inline_to_text(value)}"))
        if doc.meta:
            pdf.ln(3)
...
def _render_block_fpdf2(pdf, epw: float, block: Block) -> None:
    if block.kind == "h2":
        ...
        pdf.set_text_color(*EMBER)
        pdf.multi_cell(epw, 7.5, _safe(_md_inline_to_text(block.text)))
    ...
    if block.kind == "h3":
        ...
        pdf.set_text_color(*DARK_INK)
        pdf.multi_cell(epw, 6, _safe(_md_inline_to_text(block.text)))
        pdf.ln(1)
        return
    ...
    if block.kind == "code":
        ...
        pdf.multi_cell(epw, 4.6, _safe(block.text) or " ")
        return
    if block.kind == "table":
        pdf.set_font("Courier", "", 8.5)
        pdf.multi_cell(epw, 4.4, _safe(block.text))
        return
