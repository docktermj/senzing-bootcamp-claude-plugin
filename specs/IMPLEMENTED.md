...
- **Establishes no new invariant** — all three findings are **failures to apply INV-161/INV-162**, recorded today, to everything they bind. The rules were already right; their retro-application was incomplete.
...
- **Finding 3 (reported, NOT fixed — deferred by the maintainer):** INV-153's distinctness rule ("no two rendered labels may be identical unless their underlying values are identical") is implemented for match keys only. `senzing_viz_server.py:849` truncates graph node labels at a fixed 20 characters with no collision check, so two organizations sharing a 19-character prefix render identically; the contract mirrors the narrow scope under a heading reading "**1. Match-key labels**", so a generated non-Python server (INV-090) inherits the gap. Mitigated — the untruncated name is on hover and nodes are spatially separated. Left open as a scope question about INV-153's general clause.
