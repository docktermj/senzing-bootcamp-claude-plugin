# The absent-license branch INV-244 forbids exists in Module 4 as well, upstream of both fixed sites
...
dataset it never fires, and absence reports nothing about the installed license.
...
made, in the module that collects the data. A bootcamper whose license has no cap can be steered
...
2. **The implementation trusted the spec's enumeration instead of checking it.** The spec said the
   branch exists twice and named the two files; the implementer honored that count and swept no
...
1. In `module-04-data-collection/SKILL.md`, change the **Absent or null** branch from *fall back to
   the evaluation license* to *measure, then re-enter the three branches* — the same shape now used
...
3. Keep the evaluation-license fallback for the case where the measurement **fails**, stated as an
...
- [ ] The evaluation-license fallback survives only as the measurement-failure path, is stated as an
...
1. **Senzing facts re-asked rather than carried, as this spec required.**
   ...
   **empty** `data` array — the server documents the method, not the license payload. That matches
